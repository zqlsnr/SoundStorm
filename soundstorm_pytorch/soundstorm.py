...
class SoundStorm(nn.Module):
    ...
    def generate_by_level(
       ...
       # maybe condition
        ...
        gq = self.grouped_quantizers * self.num_quantizers
        q_seq_len = num_latents * self.grouped_quantizers

        for q, q_steps in enumerate(self.steps):
            # device and time
            times = torch.linspace(0., 1., q_steps + 1)

            # sequence starts off as all masked at q level
            lower_quantizers_mask = torch.full((batch_size, num_latents, q), False, device=device)
            q_quantizer_mask = torch.full((batch_size, num_latents, 1), True, device=device)
            upper_quantizers_mask = torch.full((batch_size, num_latents, (gq - q -1)), False, device=device)
            full_mask = torch.cat((lower_quantizers_mask, q_quantizer_mask, upper_quantizers_mask), dim=2)
            full_mask = rearrange(full_mask, 'b n q -> b (n q)')

            # slowly demask
            all_mask_num_tokens = (self.schedule_fn(times[1:]) * q_seq_len).long()
            ...
            for mask_num_tokens, steps_until_x0 in zip(all_mask_num_tokens.tolist(), reversed(range(q_steps))):
                self_cond = self.to_self_cond(last_embed) if has_self_cond else None
                logits, embeds = self.net(
                    seq,
                    cond = cond_tokens,
                ...
                logits = rearrange(logits, 'b (n q) k -> b n q k', q = gq)
                q_logits = logits[:, :, q, :]
                ...
                if exists(filter_thres):
                    q_logits = top_k(q_logits, filter_thres)

                annealing_scale = steps_until_x0 / q_steps
                temperature = start_temperature * annealing_scale

                probs = (q_logits / max(temperature, 1e-3)).softmax(dim = -1)

                sampled_ids = gumbel_sample(q_logits, temperature = max(temperature, 1e-3))

                # make sampled_ids to full_sampled_ids whoes shape is same as seq 
                full_sampled_ids = repeat(sampled_ids, 'b n -> b n q', q = gq)
                full_sampled_ids = rearrange(full_sampled_ids, 'b n q -> b (n q)')

                seq = torch.where(full_mask, full_sampled_ids, seq)
                ...
                if exists(self.token_critic):
                    ...
                    scores = 1 - q_logits.softmax(dim = -1)
                    scores = scores.gather(2, rearrange(sampled_ids, 'b n -> b n 1'))
                    scores = rearrange(scores, 'b n 1 -> b n')

                if not self.can_mask_prev_unmasked:
                    scores = scores.masked_fill(~rearrange(q_quantizer_mask, 'b n 1 -> b n'), -torch.finfo(scores.dtype).max)

                q_quantizer_mask_indices = scores.topk(mask_num_tokens, dim = -1).indices
                q_quantizer_mask = torch.zeros_like(scores, dtype = torch.bool).scatter(1, q_quantizer_mask_indices, True)

                q_quantizer_mask = rearrange(q_quantizer_mask, 'b n -> b n 1')
                full_mask = torch.cat((lower_quantizers_mask, q_quantizer_mask, upper_quantizers_mask), dim=2)
                full_mask = rearrange(full_mask, 'b n q -> b (n q)')

                seq = seq.masked_fill(full_mask, self.mask_id)
                #print(rearrange(seq, 'b (n q) -> b n q', q = gq))
    ...
    def forward(
        ...
        **kwargs
        ...
        # 如果所有数据都不替换，也就是没有True的，那么我们强制最后一列为True
        if not torch.any(replace_mask_id_mask):
            replace_mask_id_mask[:,-1] = True
